...
func TestSelectedMSIDefaultsAndNativeOverrides(t *testing.T) {
	...
	req.Metadata = raw(object{"type": "win32", "displayName": "Authored", "installCommandLine": "custom install", "uninstallCommandLine": "custom remove", "rules": []any{}})
	derived, origins, err := Derive(req)
	if err != nil {
		t.Fatal(err)
	}
	metadata, _ := decodeObject(derived.Metadata)
	if metadata["displayName"] != "Authored" || metadata["installCommandLine"] != "custom install" || len(metadata["rules"].([]any)) != 0 || origins["rules"] != "" {
		t.Fatal("MSI defaults overwrote native authoring")
	}
	req.Metadata = raw(object{"type": "win32", "unmanaged": []any{"rules", "msiInformation"}})
	derived, _, err = Derive(req)
	if err != nil {
		t.Fatal(err)
	}
	metadata, _ = decodeObject(derived.Metadata)
	if metadata["rules"] != nil || metadata["msiInformation"] != nil {
		t.Fatal("unmanaged did not suppress defaults")
	}

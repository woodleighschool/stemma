...
func TestDestinationDerivesSelectedDMGAppAndNativeOverrides(t *testing.T) {
	request := plugin.ReconcileRequest[json.RawMessage]{Prepared: true, Identity: plugin.Identity{Resource: plugin.ResourceReference{Kind: "MacSoftware", Name: "Editor"}}, Artifact: plugin.Artifact{Path: "leased.dmg", Format: "dmg"}, Subjects: map[string]plugin.SubjectSelector{"app": {Kind: "app", BundleID: "example.editor"}}, Facts: plugin.Facts{Subjects: []plugin.Subject{{Kind: "app", Path: "Editor.app", App: &plugin.AppFacts{BundleID: "example.editor", Name: "Editor", Version: "2.3", Build: "203", MinimumOS: "13.0"}}, {Kind: "app", Path: "Other.app", App: &plugin.AppFacts{BundleID: "example.other", Version: "1"}}}}, Metadata: json.RawMessage(`{"derive":{"app":{"subject":"app","version_key":"CFBundleVersion"}},"pkginfo":{"description":null,"blocking_applications":[]}}`)}
...
func TestDerivedApplicationUsesNativeDetectionKeys(t *testing.T) {
	...
	request := plugin.ReconcileRequest[json.RawMessage]{Prepared: true, Identity: plugin.Identity{Resource: plugin.ResourceReference{Kind: "MacSoftware", Name: "Editor"}}, Artifact: plugin.Artifact{Path: "leased.dmg", Format: "dmg"}, Facts: plugin.Facts{Subjects: []plugin.Subject{app}}, Metadata: json.RawMessage(`{}`)}
	derived, err := munki.Derive(request)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDerivedPackageMetadata(t *testing.T) {
	...
	request := plugin.ReconcileRequest[json.RawMessage]{Prepared: true, Identity: plugin.Identity{Resource: plugin.ResourceReference{Kind: "MacSoftware", Name: "Suite"}}, Artifact: plugin.Artifact{Path: "leased.pkg", Format: "pkg"}, Facts: facts, Metadata: json.RawMessage(`{}`)}
	derived, err := munki.Derive(request)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPKGAppRequiresKnownEndpointWhenExplicitlySelected(t *testing.T) {
	request := plugin.ReconcileRequest[json.RawMessage]{Prepared: true, Artifact: plugin.Artifact{Path: "vendor.pkg", Format: "pkg"}, Subjects: map[string]plugin.SubjectSelector{"app": {Kind: "app", BundleID: "example.app"}}, Facts: plugin.Facts{Subjects: []plugin.Subject{{Kind: "app", Path: "Payload/App.app", App: &plugin.AppFacts{BundleID: "example.app", Version: "1"}}}}, Metadata: json.RawMessage(`{"derive":{"app":{"subject":"app"}}}`)}

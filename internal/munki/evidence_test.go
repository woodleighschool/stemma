...
func TestSelectedMacApplicationEvidence(t *testing.T) {
	...
	if values["version"] != "203" || installs["CFBundleIdentifier"] != "example.editor" || installs["path"] != "/Applications/Editor.app" || installs["version_comparison_key"] != "CFBundleVersion" || decoded["supported_architectures"].([]any)[0] != "arm64" {
	...
	if values["version"] != "99" || len(values["installs"].([]any)) != 0 || values["supported_architectures"].([]any)[0] != "x86_64" {
		t.Fatal(values)
	}
}

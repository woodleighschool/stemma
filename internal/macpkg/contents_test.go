...
func TestContentsComposeIdenticallyFromTreeZIPTARAndDMG(t *testing.T) {
	...
	spec := Spec{Package: Package{Identifier: "org.example.wrapper", Version: "1.0"},
		...
		Scripts: map[string]Entry{"postinstall": {Content: &hook}, "installer": {Input: "vendor", Path: "installer"}, "config.json": {Input: "vendor", Path: "installer/config.json"}},
	}
	var digest string
...
func TestWrapperKeepsOriginalMediaAndSelectsArchiveRoot(t *testing.T) {
	...
	hook := "#!/bin/sh\nexit 94\n"
	spec := Spec{Package: Package{Identifier: "org.example.wrapper", Version: "1"}, Scripts: map[string]Entry{
		"postinstall": {Content: &hook}, "vendor.zip": {Input: "vendor"}, "expanded": {Input: "vendor", Path: ".", Mode: "0755"},
	}}
...
func TestCompositionPreservesConfinedSymlinks(t *testing.T) {
	...
	spec.Scripts["fonts"] = Entry{Input: "fonts"}
...
func TestCompositionRejectsSymlinkDestinationParents(t *testing.T) {
	...
	for name, target := range map[string]string{"alias": ".", "escape": "alias/d/../.."} {
	...
	spec.Payload = nil
	spec.Scripts["."] = Entry{Input: "fonts", Path: "."}
	spec.Scripts["escape/outside"] = Entry{Content: &content}
	_, err := Build(t.Context(), spec, inputs, t.TempDir(), time.Time{})

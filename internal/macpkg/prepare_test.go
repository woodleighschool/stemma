...
func TestPrepareEvaluatesAndCopiesTreeAndZIPWithoutChangingSources(t *testing.T) {
	...
	for _, format := range []string{"tree", "zip"} {
		t.Run(format, func(t *testing.T) {
			input := plugin.Artifact{Path: root, Tree: true, Filename: "vendor", Version: "1.2.3"}
			if format == "zip" {
				input = plugin.Artifact{Path: archive, Filename: "vendor.zip", Format: "zip", Version: "1.2.3"}
			}
			config := map[string]any{

				"package": map[string]any{"identifier": "org.example.wrapper", "version": "{{ inputs.vendor.version + '-r' + env.REVISION }}"},
			...
			if payload["Applications/Example.app/Contents/Info.plist"] != original["Example.app/Contents/Info.plist"] ||
				...
				t.Fatal("evaluation or copying changed the app or script resources")
			}
...
func TestPrepareRejectsWrongResolvedTypesAndMissingInputMetadata(t *testing.T) {
	for _, test := range []struct {
		...
		{"boolean version", "{{ true }}", "literal", "resolved package config"},
		{"numeric content", "1", "{{ 7 }}", "resolved package config"},
		{"missing input", "{{ inputs.missing.version }}", "literal", "required reference is missing"},
		{"runner path", "{{ inputs.vendor.path }}", "literal", "required reference is missing"},
	} {
		t.Run(test.name, func(t *testing.T) {
...
func TestPrepareTreatsMissingInputEvidenceAsAnEmptyObject(t *testing.T) {
	request := prepareRequest(t, map[string]any{
		...
		"scripts": map[string]any{
			"postinstall": "#!/bin/sh\nexit 96\n",
			"release":     `{{ inputs.vendor.evidence.?release.orValue("unknown") }}`,
			"count":       `{{ string(size(inputs.vendor.evidence)) }}`,
		},

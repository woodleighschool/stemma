...
spec:
  ...
  package:
    ...
    version: "{{ env.STEMMA_TEST_VERSION }}-1"
  scripts:
    postinstall: |-
      #!/bin/sh
      exit 93
      # {{ env.STEMMA_TEST_CONTENT }}
  payload:
...
`
	...
	t.Setenv("STEMMA_TEST_VERSION", "1.2.3")
	...
	for name, resource := range map[string]config.Resource{
		"build file entries": {APIVersion: "stemma/v1alpha1", Kind: "BuildMacPkg", Spec: map[string]any{
			"scripts": map[string]any{
				"resource":  "{{ env.STEMMA_TEST_RESOURCE_CONTENT }}",
				"resolver":  "{{ env.STEMMA_TEST_RESOURCE_CONTENT }}",
				"operation": "{{ env.STEMMA_TEST_RESOURCE_CONTENT }}",
			},

...
| `facts`    | Selected inspection subjects, with their native `app`, `package` or other evidence fields | After builder inspection or software preparation |
...
not runner paths. A downloaded input may have no declared version; inspect the
application when its metadata is authoritative.

[`BuildMacPkg.inspect`](building-packages.md#inspect-input-metadata) names the
subjects available in builder `facts`. Software kinds expose inspected subjects
and can name selections for destination metadata. Their fields retain the
...
Project settings, source declarations and inspection selections resolve their
environment expressions while loading. `BuildMacPkg` resolves package fields,
payload and script entries after acquiring and inspecting its inputs. Destination
metadata resolves after software preparation. Facts cannot select what must be
